...
from .satori import opcodes, push_script, base_encode, TOTAL_COIN_SUPPLY_LIMIT_IN_BTC, COIN, base_decode
...
SATORI_NAMES = "^SAT$|^SATORI$|^SATORICOIN$|^SATS$|^SATORIS$|^SATORICOINS$|^#SAT$|^#SATORI$|^#SATORICOIN$|^#SATS$|^#SATORIS$|^#SATORICOINS$|^EVR$|^EVER$|^EVRMORE$|^EVERMORE$|^EVRMORECOIN$|^EVERMORECOIN$|^EVRS$|^EVERS$|^EVRMORES$|^EVERMORES$|^EVRMORECOINS$|^EVERMORECOINS$|^#EVR$|^#EVER$|^#EVRMORE$|^#EVERMORE$|^#EVRMORECOIN$|^#EVERMORECOIN$|^#EVRS$|^#EVERS$|^#EVRMORES$|^#EVERMORES$|^#EVRMORECOINS$|^#EVERMORECOINS$|^RVN$|^RAVEN$|^RAVENCOIN$|^RVNS$|^RAVENS$|^RAVENCOINS$|^#RVN$|^#RAVEN$|^#RAVENCOIN$|^#RVNS$|^#RAVENS$|^#RAVENCOINS$"
...
def get_asset_vout_type(script: bytes) -> Optional[str]:
    ...
    sat_ptr = -1
    for op, l, ptr in ops:
        if op == opcodes.OP_SAT_ASSET:
            sat_ptr = ptr - 1
            break
    if not sat_ptr > 0:
        return None
    if script[sat_ptr+2:sat_ptr+5] == b'sat':
        sat_ptr += 5
    else:
        sat_ptr += 6
    type = bytes([script[sat_ptr]])
...
def try_get_message_from_asset_transfer(script: bytes) -> Optional[Tuple[str, Optional[int]]]:
    ...
    if type != b't':
        return None
    sat_ptr += 1
    name_len = script[sat_ptr]
    ipfs = None
    timestamp = None
    try:
        if script[sat_ptr+1+name_len+8] != 0xff:
            ipfs = script[sat_ptr+1+name_len+8:sat_ptr+1+name_len+8+34]
        if script[sat_ptr+1+name_len+8+34] != 0xff:
            timestamp = script[sat_ptr+1+name_len+8+34:sat_ptr+1+name_len+8+34+8]
...
def replace_amount_in_transfer_asset_script(script: bytes, amount: int) -> bytes:
    ...
    for op, _, ptr in ops:
        if op == opcodes.OP_SAT_ASSET:
            ...
            break
    if not sat_ptr > 0:
        raise BadAssetScript('No OP_SAT_ASSET')
    if script[sat_ptr+2:sat_ptr+5] == b'sat':
        sat_ptr += 5
    else:
        sat_ptr += 6
    ...
    sat_ptr += 1
    name_len = script[sat_ptr]
    pre_sats = script[:sat_ptr+1+name_len]
    post_sats = script[sat_ptr+1+name_len+8:]
...
def pull_meta_from_create_or_reissue_script(script: bytes) -> Dict:
    ...
    if type == b'q':
        name_len = script[sat_ptr]
        name = script[sat_ptr+1:sat_ptr+1+name_len]
        sats = script[sat_ptr+1+name_len:sat_ptr+1+name_len+8]
        divs = script[sat_ptr+1+name_len+8]
        reis = script[sat_ptr+1+name_len+8+1]
        has_i = script[sat_ptr+1+name_len+8+1+1]
        ifps = None
        if has_i != 0:
            ifps = script[sat_ptr+1+name_len+8+1+1+1:sat_ptr+1+name_len+8+1+1+1+34]
        return {
        ...
        reis = script[sat_ptr + 1 + name_len + 8 + 1]
        ifps = None
        if sat_ptr + 1 + name_len + 8 + 1 + 1 != len(script) - 1:
            ifps = script[sat_ptr + 1 + name_len + 8 + 1 + 1:sat_ptr + 1 + name_len + 8 + 1 + 1 + 34]
        return {
            'name': name.decode('ascii'),
            'sats_in_circulation': int.from_bytes(sats, 'little'),
            'divisions': divs,
...
def is_main_asset_name_good(name):
    ...
    if re.search(SATORI_NAMES, name):
        return "Main assets cannot have Ravencoin/Evrmore/Satori-like names."
