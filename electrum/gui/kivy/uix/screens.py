...
from electrum import satori, constants
...
class SendScreen(CScreen, Logger):
    ...
    def set_bolt11(self, invoice: str):
        ...
        self.amount = self.app.format_amount_and_units(lnaddr.amount * satori.COIN) if lnaddr.amount else ''
        self.payment_request = None
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a satori address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not satori.is_address(address):
                        self.app.show_error(_('Invalid Satori Address') + ':\n' + address)
                        return
