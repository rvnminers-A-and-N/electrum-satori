...
from electrum.satori import COIN, make_op_return
from electrum.util import (AssetAmountModified, UserFacingException, get_asyncio_loop, bh2u,
                           ...
                           NoDynamicFeeEstimates, InvoiceError, parse_max_spend, SatoriValue)
...
class SendTab(QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, window: 'ElectrumWindow'):
        ...
        # Let user choose to send SAT or Asset
        ...
        msg = (_("Recipient of the funds.") + "\n\n"
               + _("You may enter a Satori address, a label from your list of contacts "
                   "(a list of completions will be proposed), "
                   "or an alias (email-like address that forwards to a Satori address)") + ". "
    ...
    def spend_max(self):
        ...
        __, x_fee_amount = run_hook('get_tx_extra_fee', self.wallet, tx) or (None, SatoriValue())
        ...
        if amount_after_all_fees.assets:
            ...
            amount_after_all_fees = amount_after_all_fees.sat_value.value

        self.amount_e.setAmount(amount_after_all_fees)
        # show tooltip explaining max amount
        mining_fee = tx.get_fee()
        # Mining fees will only be sat
    ...
    def pay_onchain_dialog(
        ...
        output_values = [x.satori_value for x in outputs]
        if any(parse_max_spend(outval) for outval in output_values):
            output_value = '!'
        else:
            output_value = sum(output_values, SatoriValue())
    ...
    def get_frozen_balance_str(self) -> Optional[str]:
        frozen_bal = sum(self.wallet.get_frozen_balance(), SatoriValue())
        if frozen_bal == SatoriValue():
            return None
