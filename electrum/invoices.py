...
from .satori import address_to_script
from .transaction import PartialTxOutput, SatoriValue
...
class Invoice(StoredObject):
    ...
    def generate_amount(x):
        if isinstance(x, SatoriValue):
            return x
        return SatoriValue.from_json(x)
    ...
    def get_asset(self) -> Optional[str]:
        return next(iter(self.outputs[0].satori_value.assets.keys()), None)
    ...
    def get_amount_msat(self) -> Union[SatoriValue, None]:
    ...
    def get_amount_sat(self) -> Union[SatoriValue, None]:
        """
    ...
    def get_bip21_URI(self, lightning=None):
        ...
        if amount is not None:
            amount = amount.sat_value.value if not asset else amount.assets[asset].value
        ...
        if not addr and lightning:
            return "satori:?lightning="+lightning
    ...
    def _validate_amount(self, attribute, value: Optional[SatoriValue]):
        if value is None:
            return
        if value.assets:
            assert value.sat_value == 0
        ...
        if isinstance(value.sat_value, (Satoshis, int)):
            if not (0 <= value.sat_value <= TOTAL_COIN_SUPPLY_LIMIT_IN_BTC * COIN * 1000):
                raise InvoiceError(f"amount is out-of-bounds: {value!r} msat")
        elif isinstance(value.sat_value, str):
            if value.sat_value != '!':
