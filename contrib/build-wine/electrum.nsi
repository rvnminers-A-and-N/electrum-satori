...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Satori"
  !define PRODUCT_WEB_SITE "https://github.com/SatoriNetwork/electrum-satori"
  !define PRODUCT_PUBLISHER "The Satori Community and Echelon Technology Group"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-satori-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum\gui\icons\electrum-satori.ico"
...
Section
  ...
  File "..\..\electrum\gui\icons\electrum-satori.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-satori-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-satori-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-satori-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  ;WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-satori.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-satori-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning" "DefaultIcon" "$\"$INSTDIR\electrum-satori.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-satori-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-satori.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\satori"
